...
def build_messages(text_to_analyze: str, instruction_prompt: str) -> List[dict]:
    """构建 DeepSeek Chat 消息。"""
    system_msg = (
        "你是一名专业的首席投资分析师，擅长从海量信息中提取 actionable insights。\n"
        "核心要求：\n"
        "• 数据驱动：每个结论必须有文本依据\n"
        "• 风险意识：每项建议必须配套风险提示\n"
        "• 可执行性：建议要具体到板块/标的/时机\n"
...
def generate_ai_html_from_md(md_text: str, output_html_path: Path) -> Path:
    ...
    * { box-sizing: border-box; }
    ...
    html = (
        ...
        "        const toRemove = []; let builtCards = false;\n"
        ...
        "        const removeList = []; let builtCards = false;\n"
        ...
        "              sec.appendChild(card); builtCards = true;\n"
        ...
        "          node = node.nextElementSibling;\n"
        "        }\n"
        "        if (builtCards && removeList.length) {\n"
